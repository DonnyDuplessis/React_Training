{
    ...
    const exampleArray: string[] = ['sfa', 'fafa', 'atea']
    exampleArray.push('aa')

    const [name1, name2] = exampleArray

    const exampleArrayCopy = [...exampleArray, 'test', 'hi']

    //

    // myinput: HTMLInputElement = document.getElementById("transactions") as HTMLInputElement
    const transactionArray: Number[] = [123, 567]

    const newTransaction = (input) => {
        const myinput: HTMLInputElement = document.getElementById("transactions") as HTMLInputElement
        transactionArray.push(Number(myinput.value))
